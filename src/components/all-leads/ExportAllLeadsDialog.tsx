...
import { allLeadsToExportCsv } from "@/lib/all-lead-csv";
...
const SHEET_EXPORT_OPTIONS: {
  ...
  { value: "old", label: "Old Leads" },
];

function leadMatchesSheetExport(l: AllLeadDocument, sheet: "today" | "old") {
  return l.sheetTab === sheet;
}

function filterLeadsForExport(
  list: AllLeadDocument[],
  opts: {
    dateFrom: string;
    dateTo: string;
    course: string;
...
type Props = {
  ...
  leads: AllLeadDocument[];
  open: boolean;
  onOpenChange: (open: boolean) => void;
};

export function ExportAllLeadsDialog({ leads, open, onOpenChange }: Props) {
  ...
  const [sheet, setSheet] = useState<"today" | "old">("today");
  ...
  const runExport = () => {
    ...
    const csv = allLeadsToExportCsv(rows as unknown as Parameters<typeof allLeadsToExportCsv>[0]);
    ...
    a.href = url;
    const sheetTag = listFilterEnabled ? sheet : "all-lists";
    const tag = [
      sheetTag,
      dateFrom || "any",
      dateTo || "any",
    ...
    a.download = `all-leads-export_${tag}_${format(new Date(), "yyyy-MM-dd-HHmm")}.csv`;
    a.click();
  ...
  return (
    <dialog
        ...
        aria-labelledby="export-all-leads-title"
      >
        ...
        <div className="max-h-[min(60vh,480px)] overflow-y-auto px-4 py-4 [scrollbar-width:thin]">
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <p className="text-[12px] font-semibold text-slate-900">
                Limit export to one tab
              </p>
              <p className="mt-0.5 text-[11px] leading-snug text-slate-600">
                Turn on to match a tab (Today&apos;s or Old). Off =
                every tab is included (still filtered by dates / course /
                country below).
              </p>
            </div>
          </label>
          ...
          >
            ...
            <select
              ...
              onChange={(e) =>
                setSheet(e.target.value as "today" | "old")
              }
            >
