...
import {
  ALL_LEAD_CSV_EXPORT_HEADERS,
  matrixToStringGrid,
  parseCsvText,
  parseAllLeadImportRows,
  type ParsedImportAllLead,
  downloadAllLeadCsvTemplateFile,
} from "@/lib/all-lead-csv";
import { formatTargetExams } from "@/lib/lead-display";
import { cn } from "@/lib/cn";
import { SX } from "@/components/student/student-excel-ui";

export type ImportAllLeadExcelControlHandle = {
  open: () => void;
};

type Props = {
  /** Refetch all leads after a successful import. */
...
export const ImportAllLeadExcelControl = forwardRef<
  ImportAllLeadExcelControlHandle,
  Props
>(function ImportAllLeadExcelControl(
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const { leads, issues } = parseAllLeadImportRows(grid);
        if (!cancelled) {
  ...
  const runImport = useCallback(async () => {
    ...
    setMessage(null);
    try {
      const res = await fetch("/api/all-leads/batch", {
        method: "POST",
  ...
  return (
    <>
      ...
      <dialog
        ...
        aria-labelledby="import-all-leads-title"
      >
