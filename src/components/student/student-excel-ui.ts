 ...
 */
export const SX = {
  pageWrap: "flex flex-col gap-0 pb-12",

  outerSheet:
    "overflow-hidden rounded-none border border-slate-200 bg-white shadow-sm shadow-slate-900/[0.04]",

  toolbar:
    "flex flex-wrap items-center gap-x-3 gap-y-1 border-b border-slate-200 bg-slate-50 px-3 py-2",

  toolbarTitle: "text-[13px] font-semibold tracking-tight text-slate-900",
  toolbarMeta: "text-[12px] text-slate-500",

  /** Student detail hero — calm hierarchy, minimal chrome */
  studentHero: "border-b border-slate-100 bg-white",
  studentHeroTop:
    "flex flex-wrap items-center gap-x-3 gap-y-1 px-4 pt-5 sm:px-8",
  studentHeroBack:
    "text-[13px] font-medium text-primary transition-colors hover:text-[#0d47a1] hover:underline",
  studentHeroMetaTop: "text-[13px] text-slate-500",
  studentHeroBody: "px-4 pb-8 pt-2 sm:px-8",
  studentHeroTitleRow:
    "flex flex-wrap items-start justify-between gap-4 gap-y-3",
  studentHeroName:
    "text-xl font-semibold leading-tight tracking-tight text-slate-900 sm:text-2xl",
  studentHeroIconRow:
    "mt-5 flex flex-wrap items-center gap-x-8 gap-y-3 text-[14px]",
  studentHeroIconItem: "inline-flex items-center gap-2.5 text-slate-600",
  studentHeroIcon: "h-5 w-5 shrink-0 text-slate-400",
  studentHeroCourseBadge:
    "inline-flex items-center rounded-md bg-primary/10 px-2.5 py-0.5 text-xs font-medium text-primary",
  studentHeroSubline: "mt-5 text-[13px] leading-relaxed text-slate-600",
  studentHeroSubLabel: "text-slate-500",
  studentHeroSubVal: "font-medium text-slate-900",

  kvWrap: "border-b border-slate-100 bg-white",
  kvTable: "w-full border-collapse text-[13px]",
  kvTh:
    "w-[min(32%,160px)] whitespace-nowrap border border-slate-200 bg-slate-50 px-2.5 py-1.5 text-left text-[11px] font-semibold uppercase tracking-wide text-slate-600",
  kvTd:
    "border border-slate-200 bg-white px-2.5 py-1.5 text-slate-800 align-middle",

  /** Lead summary ribbon — labels are chips (field names), values read as data. */
  summaryRow:
    "flex min-h-[40px] w-full min-w-0 flex-nowrap items-stretch overflow-x-auto border-b border-slate-100 bg-slate-50/50 text-[13px] leading-tight [scrollbar-width:thin]",
  summarySeg:
    "flex shrink-0 items-center gap-2 border-r border-slate-100 bg-white px-2.5 py-2 last:border-r-0",
  ...
  stepperShell: "border-b border-slate-100 bg-white",
  stepperTrack: "bg-slate-100 px-px pt-px",
  stepperGrid: "grid grid-cols-5 gap-px bg-slate-200 p-px",
  stepperStatusBar:
    "flex flex-wrap items-center justify-between gap-x-4 gap-y-1 border-t border-slate-100 bg-slate-50/80 px-3 py-2 text-[12px] text-slate-600",

  mainSplit: "flex flex-col bg-slate-50/70 lg:flex-row lg:items-stretch",
  mainPane:
    "flex min-h-0 min-w-0 flex-[1] flex-col bg-white p-2 sm:p-3 lg:border-r lg:border-slate-100",
  asidePane:
    "w-full shrink-0 border-t border-slate-100 bg-white lg:w-[min(30%,320px)] lg:min-w-[260px] lg:border-l lg:border-t-0",

  section:
    "scroll-mt-20 overflow-hidden rounded-none border border-slate-200 bg-white shadow-none",
  sectionHead:
    "flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 bg-white px-3 py-2 sm:px-4",
  sectionTitle: "text-sm font-semibold tracking-tight text-slate-900",
  sectionBody: "p-3 sm:p-4",

  dataTable: "w-full border-collapse text-[13px]",
  dataTh:
    "border border-slate-200 bg-slate-50 px-2.5 py-2 text-left text-[11px] font-semibold uppercase tracking-wide text-slate-600",
  dataTd:
    "border border-slate-200 bg-white px-2.5 py-2 align-middle text-[13px] text-slate-800",
  dataTdMuted:
    "border border-slate-200 bg-white px-2.5 py-2 align-middle text-[13px] text-slate-500",

  zebraRow: "bg-slate-50/60",

  input:
    "h-9 w-full rounded-none border border-slate-200 bg-white px-2.5 text-[13px] text-slate-900 shadow-none focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary/25",
  textarea:
    "w-full rounded-none border border-slate-200 bg-white px-2.5 py-2 text-[13px] leading-relaxed text-slate-900 shadow-none focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary/25",
  select:
    "h-9 rounded-none border border-slate-200 bg-white px-2.5 text-[13px] text-slate-900 shadow-none focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary/25",

  sidePanel: "border-b border-slate-100 bg-white last:border-b-0",
  sideHead:
    "border-b border-slate-100 bg-slate-50/90 px-4 py-2.5 text-xs font-semibold text-slate-700",
  sideBody: "p-4",
  asideIntro:
    "border-b border-slate-100 px-4 py-3 text-xs leading-relaxed text-slate-500",
  ...
  footerBar:
    "grid grid-cols-[1fr_auto_1fr] items-center gap-2 border-t border-slate-100 bg-white px-3 py-2.5",
