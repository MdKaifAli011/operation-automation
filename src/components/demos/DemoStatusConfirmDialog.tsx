...
export function DemoStatusConfirmDialog({ demo, newStatus, onConfirm, onCancel }: Props) {
      ...
      <div className="w-full max-w-md rounded-none border border-slate-200 bg-white p-6 shadow-xl">
        ...
        <div className="flex justify-end gap-3">
          <button
            type="button"
            onClick={onCancel}
            className="h-9 px-4 text-[13px] font-semibold text-slate-700 hover:bg-slate-100 rounded-none transition-colors"
          >
            Cancel
          </button>
          <button
            type="button"
            onClick={onConfirm}
            className="h-9 px-4 text-[13px] font-semibold text-white bg-emerald-600 hover:bg-emerald-700 rounded-none transition-colors"
          >
            Confirm
          </button>
        </div>
